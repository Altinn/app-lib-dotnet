...
namespace Altinn.Common.EFormidlingClient
{
    ...
    public class EFormidlingClient : IEFormidlingClient
    {
        ...
        public async Task FindOutGoingMessages(string serviceIdentifier, Dictionary<string, string> requestHeaders)
        {
            ...
            try
            {
                HttpResponseMessage response = await _client.GetAsync(
                    $"messages/out/?serviceIdentifier={serviceIdentifier}",
                    requestHeaders
                );
        ...
        /// <inheritdoc/>
        public async Task<bool> UploadAttachment(
            Stream stream,
            string id,
            string filename,
        ...
        )
        {
            ...
            HttpResponseMessage response = await _client.PutAsync(
                $"messages/out/{id}?title={filename}",
                streamContent,
                requestHeaders
            );
            ...
            {
                _logger.LogError(
                    $"The remote server returned unexpcted status code: {response.StatusCode} - {responseBody}."
                );
        ...
        )
        {
            ...
            try
            {
                ...
                StandardBusinessDocument sbdVerified = JsonSerializer.Deserialize<StandardBusinessDocument>(
                    responseBody
                );
                _logger.LogDebug(responseBody);
        ...
        /// <inheritdoc/>
        public async Task<bool> SubscribeeFormidling(
