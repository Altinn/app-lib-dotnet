...
public abstract class RepeatingReferenceComponent : BaseComponent
{
    ...
    )
    {
        ...
        var dataModel = new DataModel(dataAccessor);
        var rowCount =
            await dataModel.GetModelDataCount(GroupModelBinding, defaultDataElementIdentifier, rowIndexes) ?? 0;
        ...
        for (int i = rowCount - 1; i >= 0; i--)
        {
            ...
            childContexts.Add(
                new ComponentContext(rowComponent, subRowIndexes, defaultDataElementIdentifier, rowChildren)
            );
        }

        return new ComponentContext(this, rowIndexes, defaultDataElementIdentifier, childContexts);
    }
}
