            ...
            builder.Append(
                $$"""
                            case "{{child.JsonPath}}":
                                ...
                                bufferOffset += {{child.JsonPath.Length}};
                ...
                builder.Append(
                    """
                                    ...
                                    if (path.Length <= pathOffset && path[pathOffset] == '[')
                                    {
                                        ...
                                        PathHelper.GetIndex(path, pathOffset, out int nextPathIndexOffset);
            ...
            builder.Append(
                $$"""
                            case "{{child.JsonPath}}":
                                segment.CopyTo(buffer.Slice(bufferOffset));
                                bufferOffset += {{child.JsonPath.Length}};
                                pathOffset += {{child.JsonPath.Length}};
