...
public class InstancesController_CopyInstanceTests
{
    ...
    [Fact]
    public async Task CopyInstance_BinaryDataCopyFails_ContinuesWithOtherData()
    {
        // Arrange
        using var fixture = InstancesControllerFixture.Create();
        const string Org = "ttd";
        ...
        // Form data mocks (should succeed)
        fixture
            .Mock<IDataClient>()
            .Setup(p =>
                p.GetFormData(instanceGuid, It.IsAny<Type?>()!, Org, AppName, InstanceOwnerPartyId, formDataGuid)
        ...
        // Assert
        Assert.IsType<RedirectResult>(actual); // Should still succeed overall

        // Verify form data was copied (should succeed despite binary failure)
        fixture
            .Mock<IDataClient>()
            .Verify(
                p => p.GetFormData(instanceGuid, It.IsAny<Type?>()!, Org, AppName, InstanceOwnerPartyId, formDataGuid),
